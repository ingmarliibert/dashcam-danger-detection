...
def detect_triangles(frame, original):
    traffic_sign = traffic_sign_factory()
    grey_frm = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    ...
    for c in contours:
        ...
        if len(approx) == 3:
            ...
            # crop image and submit to model analysis
            # bounding rectangle
    rows = frame.shape[0]
